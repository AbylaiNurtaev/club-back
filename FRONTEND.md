...
- Тело: `{ "clubId": "482917", "phone": "79991234567" }`  
  (или `"phone": "+7 999 123-45-67"` — бэкенд сам нормализует)
- Ответ: `{ spin: { _id, prize: { name, image, slotIndex, ... }, playerPhone }, newBalance }`  
- Ошибки: `404` — клуб не найден / пользователь не найден; `400` — мало баллов или приз закончился.

Итого: пользователь вводит **6-значный код** и **телефон** → вызываешь спин по телефону → показываешь приз и новый баланс.
...
| Крутить по телефону | POST | `/api/players/spin-by-phone` | `{ "clubId": "482917", "phone": "79991234567" }` |
