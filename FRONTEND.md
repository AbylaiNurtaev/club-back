...
- Ошибки: `400` — «Передайте latitude и longitude» / «Вы не в радиусе ни одного клуба (в пределах 200 м)» / мало баллов; `404` — пользователь не найден; **`429`** — рулетка этого клуба занята (кто-то только что крутил). Тело ответа: `{ message: "Рулетка занята, попробуйте позже", code: "ROULETTE_BUSY", retryAfterSeconds: <секунд до разблокировки> }`. Рекомендуется показывать пользователю сообщение и блокировать кнопку «Крутить» на `retryAfterSeconds` секунд (например, таймер обратного отсчёта).
...
| Крутить по телефону | POST | `/api/players/spin-by-phone` | `{ "phone": "79991234567", "latitude": 43.24, "longitude": 76.95 }` (clubId не обязателен). При 429: «Рулетка занята» — показать сообщение и отключить кнопку на `retryAfterSeconds` сек |
