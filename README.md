...
- Создание и управление призами (до 35 слотов)
...
**WebSocket (Socket.IO) — новые спины без polling**

Подключение к тому же хосту и порту, что и REST API. При новом спине в клубе сервер шлёт событие в комнату клуба — фронт может слушать его вместо опроса раз в 2 секунды.

- **Подключение:** `io(API_URL, { query: { clubId: "<club._id | clubId | qrToken>" } })`
- После подключения сокет автоматически добавляется в комнату `club:<mongo _id>` (клуб ищется по `_id`, `clubId` или `qrToken`).
- **Событие:** `spin` — payload как у ответа POST /api/players/spin (поле `spin`), плюс `playerPhone` для чата выигрышей.
